...
namespace ecstl {
    ...
    class View2<std::tuple<Pools...> >: public std::ranges::view_interface<View2<std::tuple<Pools... > > >{
    public:
        ...
        public:
            ...
            constexpr Iterator(View2 *owner, Iterators iters, std::size_t master)
            ...
            constexpr bool operator==(const Sentinel &) const {
                ...
                sequence_iterate<std::tuple_size_v<Iterators> >([&](auto idx){
                    if (idx == _master) is_end = std::get<idx>(_iters) == std::get<PoolsTuple>(_owner->_pools)->end();                                        
                });
                return is_end;
            }
        ...
        public:
            View2 *_owner = nullptr;
            Iterators _iters = {};
            std::size_t _master = 0;
            union {
                Values  _cache;                
            };
            bool _cache_filled = false;

            constexpr void fill_cache() const {
                const Entity &ent = std::get<0>(_iters)->first;
                _cache = std::apply([&](auto &... iters){return std::tie(ent,iters->second...);}, _iters);
            }
        ...
        constexpr View2(PoolsTuple pools): _pools(pools){}
        ...
        friend constexpr Iterator begin(View2 const &v) noexcept { return v.begin(); }
        friend constexpr Iterator end(View2 const &v) noexcept { return v.end(); }
